function PhysicsText( string , params ){
  ...
  this.params = _.defaults( params || {} , {
    ...
    sim: shaders.simulationShaders.text,
  ...
  this.particles = textParticles.createTextParticles( this.string );
  ...
  gRepelPos.push( camera.position );
  gRepelPos.push( camera.position );
  gRepelPos.push( camera.position );
  gRepelPos.push( camera.position );
  ...
  this.pagePos       = { type:"v3" , value: new THREE.Vector3() }
  ...
  this.physics.setUniform( 'timer'       , time               );
